...
const SelectionHandle = ({ className }: { className?: string }) => (
  <div className={`absolute w-2 h-2 bg-white border border-design-blue z-20 ${className}`} />
);

const WindowHeader = ({ title, color = "bg-gray-600" }: { title: string, color?: string }) => (
  <div className="h-8 bg-[#2A2A2A] border-b border-white/5 flex items-center px-3 justify-between rounded-t-xl select-none">
    ...
    <span className="text-[10px] font-mono text-gray-400 opacity-70 flex-1 text-center mr-10">{title}</span>
  </div>
);
...
const Tag: React.FC<TagProps> = ({ text, color }) => (
  <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider border border-white/10 ${color}`}>
    {text}
  </span>
);
...
const N8nVisual = () => {
  return (
    <div className="relative w-full h-full bg-[#181818] rounded-b-xl overflow-hidden p-4 grid-bg">
      ...
      >
         <div className="text-[9px] text-design-orange font-bold mb-1">WEBHOOK</div>
      ...
      >
         <div className="flex justify-between items-center mb-1">
            <div className="text-[9px] text-design-purple font-bold">AI AGENT</div>
            <div className="text-[8px] bg-design-purple text-white px-1 rounded">GPT-4</div>
         </div>
         <div className="h-1 w-full bg-gray-700 rounded mb-1"></div>
         <div className="h-1 w-full bg-gray-700 rounded mb-1"></div>
      ...
      >
         <div className="text-[9px] text-green-400 font-bold mb-1">GO SHEETS</div>
         <div className="h-1 w-full bg-gray-700 rounded"></div>
    ...
    <div className="relative w-full h-full bg-[#0D1117] rounded-b-xl overflow-hidden p-6 flex items-center justify-center">
       ...
       >
          <div className="flex gap-2 mb-2 border-b border-white/5 pb-2">
            <span className="text-blue-400">const</span> <span className="text-yellow-200">eventApp</span> = <span className="text-blue-400">new</span> <span className="text-green-400">VibeApp</span>();
          </div>
          <div className="pl-4 space-y-1">
            <div><span className="text-purple-400">eventApp</span>.<span className="text-blue-300">onScan</span>((<span className="text-orange-300">code</span>) ={'>'} {'{'}</div>
            <div className="pl-4"><span className="text-gray-500">// Auto-tagging GTM</span></div>
            <div className="pl-4"><span className="text-yellow-200">GTM</span>.<span className="text-blue-300">push</span>({'{'} event: <span className="text-green-300">'offline_checkin'</span> {'}'});</div>
            <div className="pl-4"><span className="text-yellow-200">CMS</span>.<span className="text-blue-300">validate</span>(<span className="text-orange-300">code</span>);</div>
            <div>{'}'});</div>
          </div>
       </motion.div>
       ...
       >
          ...
          <div className="w-full h-full bg-gradient-to-br from-green-900 to-black p-3 pt-8 flex flex-col items-center">
             <div className="w-12 h-12 bg-white/10 rounded-full flex items-center justify-center mb-4 backdrop-blur-md">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><path d="M3 3h6v6H3zM14 3h7v7h-7zM14 14h7v7h-7zM3 14h6v6H3z"/></svg>
             </div>
             <div className="text-white text-[10px] font-bold mb-1">QR SUCCESS</div>
             <div className="text-green-400 text-[8px] mb-4">You are checked in!</div>
    ...
    <div className="relative w-full h-full bg-[#1E1E2E] rounded-b-xl overflow-hidden p-4 grid grid-cols-2 gap-4">
       ...
       <div className="flex flex-col gap-2 relative z-10">
         <div className="h-20 bg-[#252535] rounded border border-white/5 p-2 relative overflow-hidden shadow-lg">
            <div className="text-[8px] text-gray-500 mb-1">PIPELINE_CONFIG.YAML</div>
       ...
       <div className="flex flex-col gap-2 relative z-10">
          ...
          <div className="flex-1 bg-[#252535] rounded border border-white/5 p-2 flex flex-wrap content-start gap-1 shadow-lg">
             <div className="w-full text-[8px] text-gray-400 mb-1">RFM SEGMENTS</div>
             <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} className="w-6 h-6 bg-red-500/80 rounded-sm" />
             <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} transition={{ delay: 0.1 }} className="w-10 h-6 bg-orange-500/80 rounded-sm" />
             <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} transition={{ delay: 0.2 }} className="w-4 h-6 bg-yellow-500/80 rounded-sm" />
             <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} transition={{ delay: 0.3 }} className="w-8 h-6 bg-green-500/80 rounded-sm" />
             <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} transition={{ delay: 0.4 }} className="w-5 h-6 bg-blue-500/80 rounded-sm" />
          </div>
          
          {/* Stats Bar */}
          <div className="h-10 bg-[#252535] rounded border border-white/5 flex items-center px-2 gap-2 shadow-lg">
             <div className="flex-1 h-1 bg-gray-700 rounded-full overflow-hidden">
               <motion.div initial={{ width: 0 }} whileInView={{ width: '70%' }} className="h-full bg-gradient-to-r from-blue-500 to-purple-500" />
             </div>
...
const ProjectCard: React.FC<ProjectCardProps> = ({ 
  id, title, subtitle, description, tags, color, VisualComponent, align = 'left', className 
}) => {
  ...
  return (
    ...
    >
      {/* Visual Side (Span 7) */}
      <div 
        className={`lg:col-span-7 h-[300px] md:h-[400px] relative group cursor-none ${align === 'right' ? 'lg:order-2' : 'lg:order-1'}`}
      ...
      {/* Content Side (Span 5) */}
      <div className={`lg:col-span-5 flex flex-col justify-center ${align === 'right' ? 'lg:order-1 lg:text-right items-end' : 'lg:order-2 lg:text-left items-start'}`}>
         
         <div className={`flex items-center gap-3 mb-4 ${align === 'right' ? 'flex-row-reverse' : 'flex-row'}`}>
            <span className={`text-4xl md:text-5xl font-display font-black opacity-10 select-none text-white`}>
               {id}
            </span>
         ...
         <p className={`${color.replace('bg-', 'text-')} text-sm font-bold uppercase tracking-widest mb-6`}>
           {subtitle}
         </p>
         ...
         <div className={`flex flex-wrap gap-2 mb-8 ${align === 'right' ? 'justify-end' : 'justify-start'}`}>
           {tags.map(tag => (
             <Tag key={tag} text={tag} color="text-gray-300 bg-white/5 hover:bg-white/10 transition-colors" />
           ))}
         </div>
    ...
    <section id="projects" className="py-32 px-4 relative overflow-hidden">
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="mb-40 relative flex justify-center">
            ...
            <div className="relative">
                ...
                <div className="relative text-center select-none">
                    ...
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[15rem] md:text-[25rem] font-display font-bold leading-none text-transparent stroke-text opacity-[0.03] pointer-events-none z-0" 
                    ...
                    >
                        ...
                        {/* Connecting Arrow */}
                        <svg className="absolute top-full left-1/2 w-8 h-8 text-design-pink" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    ...
                    >
                         ...
                         {['P', 'R', 'O', 'J', 'E', 'C', 'T', 'S'].map((char, i) => (
                             <motion.span 
                                ...
                                whileHover={{ y: -20, color: i % 2 === 0 ? '#3b82f6' : '#8b5cf6' }} // Blue/Purple alternate
                ...
                {/* Subtext */}
                 ...
                 >
                    A collection of solutions focusing on <span className="text-white font-medium">Automation</span>, <span className="text-white font-medium">Product Engineering</span>, and <span className="text-white font-medium">Data Intelligence</span>.
                </motion.p>
            </div>
        </div>
        {/* === END HEADER === */}
        ...
        <ProjectCard 
          ...
          tags={['React Native', 'GTM/GA4', 'Node.js', 'Internal Tools', 'App Dev']}
