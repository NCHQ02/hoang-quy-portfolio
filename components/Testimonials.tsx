...
const CommentPin = ({ color, initials }: { color: string, initials: string }) => (
  <div className="relative">
     {/* The Pin Shape */}
     <div className={`w-8 h-8 rounded-full rounded-bl-none ${color} border-2 border-white flex items-center justify-center shadow-lg relative z-10`}>
        ...
        >
            <div className="relative">
                ...
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="drop-shadow-md">
                    <path d="M3 3L10.07 19.97L12.58 12.58L19.97 10.07L3 3Z" fill={getHex(feedback.color)} stroke="white" strokeWidth="1.5" strokeLinejoin="round"/>
                </svg>
                <div className={`absolute left-4 top-4 ${feedback.color} text-white text-[10px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap border border-white/20`}>
                    {feedback.name}
                </div>
            </div>
        </motion.div>
        ...
        <div className="bg-[#2C2C2E] border border-white/10 rounded-xl p-4 shadow-2xl relative hover:border-white/20 transition-colors">
            ...
            <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h4 className="text-sm font-bold text-white leading-none mb-1">{feedback.name}</h4>
                        <p className="text-[10px] text-gray-500 font-mono">{feedback.role}</p>
                    </div>
                </div>
            ...
            <div className="border-t border-white/5 pt-3 flex items-center justify-between">
                 <div className="flex -space-x-1.5">
                     ...
                     <div className="w-5 h-5 rounded-full bg-gray-400 border border-[#2C2C2E] flex items-center justify-center text-[8px] text-black font-bold">+1</div>
                 </div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
            ...
            <div className="mb-40 relative flex flex-col items-center justify-center select-none">
                ...
                >
                    <div className="flex -space-x-3">
                        {FEEDBACKS.map((fb, i) => (
                        <div key={i} className={`w-8 h-8 rounded-full border-2 border-[#1e1e1e] flex items-center justify-center text-[10px] font-bold text-black shadow-sm transform group-hover:translate-x-1 transition-transform`} style={{ backgroundColor: fb.color.includes('#') ? fb.color.replace('bg-[', '').replace(']', '') : '#888' }}>
                            {fb.initials}
                        </div>
                        ))}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-gray-300 uppercase tracking-wide">Live Review</span>
                    </div>
                </motion.div>
                        ...
                        <div className="relative inline-block">
                            ...
                            >
                                <div className="relative group/stamp cursor-pointer">
                                    ...
                                    {/* Star Decoration */}
                                    <div className="absolute -top-3 -right-3 text-2xl md:text-4xl">✨</div>
                                </div>
                            </motion.div>
                        </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8 md:gap-12 lg:px-20 relative">
                ...
                {FEEDBACKS.map((fb, idx) => (
                    <div 
                        key={fb.id} 
                        className={`flex ${idx % 2 === 0 ? 'md:justify-end' : 'md:justify-start'} ${idx === 1 ? 'md:mt-12' : ''} ${idx === 2 ? 'md:-mt-12' : ''}`}
                    >
            ...
            >
                <div className="inline-block relative group">
                    ...
                    <div className="absolute -right-8 top-1/2 translate-x-full -translate-y-1/2 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="text-design-blue -rotate-12">
                            <path d="M3 3L10.07 19.97L12.58 12.58L19.97 10.07L3 3Z" fill="currentColor" stroke="white" strokeWidth="2" strokeLinejoin="round"/>
                        </svg>
                        <span className="bg-design-blue text-white text-[9px] px-1.5 py-0.5 rounded ml-2">You</span>
                    </div>
                </div>
            </motion.div>
